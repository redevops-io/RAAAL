"""Both witnesses through one loop, and what fusion made of them.

**This file tested an implementation that no longer exists.**

`src/discovery/pipeline.py` was deleted when Quantify's serving path moved to
...
pytest.skip(
    "tests src/discovery/pipeline.py, deleted with the migration to "
