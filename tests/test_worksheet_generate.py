...
class TestTheJourney:
    ...
    def test_saving_a_plan_produces_a_worksheet(self, client):
        ...
        response, plan_id = submit_rendered_confirmation(api_client, COMPLETE)
        assert response.status_code == 303, response.text
        assert plan_id and plan_id.startswith("plan-"), (
            "the identity must be generated by the server; the form once "
            "hardcoded plan_id=my-plan, so a user could never save a second "
            "plan")

        worksheet = store.worksheet_for_scenario(plan_id, OWNER)
    ...
    def test_the_worksheet_cites_a_run_that_exists(self, client):
        ...
        _response, plan_id = submit_rendered_confirmation(api_client, COMPLETE)

        record = store.worksheet_for_scenario(plan_id, OWNER)
    ...
    def test_the_saved_worksheet_opens(self, client):
        ...
        identifier = _store.worksheet_for_scenario(plan_id, OWNER)["worksheet_id"]
